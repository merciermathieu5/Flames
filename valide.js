/* Harnais de validation — Flames de Calgary (jsdom) */
const fs = require('fs');
...
const ATTENDU = {
  equipe: 'CALGARY',
  taille: 24,
  echantillon: {nom: 'Seth Jones', salaire: 6250000, ct: 3, ov: 82},
  masseSousContrat: 42150000,
  nbSousContrat: 9
};
...
(async () => {
  ...
  const vRei = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Griffin Reinhart'), 'UFA', 1);
  egal(vRei.code, 'surpaye', 'Griffin Reinhart (OV 81, 32 ans, 7,2 M, UFA) → Surpayé');
  egal(vRei.minEff, 6250000, 'Charte Reinhart : 6 250 000 $');
  egal(vRei.ecart, 950000, 'Écart Reinhart : +950 000 $');
  const vSnu = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Jimmy Snuggerud'), 'RFA', 1);
  egal(vSnu.code, 'aubaine', 'Jimmy Snuggerud (OV 78, 22 ans, 900 k, RFA) → Aubaine salariale');
  egal(vSnu.ecart, 1350000, 'Écart Snuggerud : −1 350 000 $ sous la charte (2,25 M)');
  ...
  const vWal = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Jesper Wallstedt'), 'RFA', 1);
  egal(vWal.minEff, 3750000, 'Jesper Wallstedt (G, OV 80, 24 ans, RFA) évalué à l\'échelon 79 : 3 750 000 $');
  egal(vWal.code, 'aubaine', 'Jesper Wallstedt à 1,875 M → Aubaine salariale');
  ...
  selJoueur.value = 'Griffin Reinhart';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Reinhart sélectionné : badge Surpayé');
  ...
  egal(celluleActive.textContent.replace(/[\s\u00a0\u202f]/g,''), '6250$',
    'Cellule active : 6 250 $ (UFA 34-, OV 81, Reinhart)');
  egal(doc.getElementById('csStatut').value, 'UFA', 'Charte UFA proposée d\'après l\'âge (32 ans)');
  ...
  selJoueur.value = 'Jesper Wallstedt';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('aubaine'), 'Jesper Wallstedt : badge Aubaine salariale');
  ok(doc.querySelector('#csFiche').textContent.includes('79'), 'Échelon gardien (OV −1 → 79) affiché dans la fiche');
